...
description: Publish one HTML page through Onimi Pages Remote MCP / 通过 Onimi Pages 远程 MCP 发布一个 HTML 页面。Use when the user asks to upload, share, preview, or publish HTML.
metadata:
  version: "0.2.1"
---
...
Keep URLs, project identifiers, tool names, and machine visibility values exact rather than translating
them. The page's content language is a separate choice and follows the user's request.
...
- Treat an HTTP 400 or 403 upload response as the server's actual rejection, not as a temporary network
  failure. Preserve the request ID, selected project, and failed stage when reporting it. Do not claim
  that dashboard upload will bypass the same storage or authorization boundary; offer it only when the
  returned error or current product documentation establishes that it is a valid alternative.
